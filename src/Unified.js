...
const UnifiedService = () => {
  ...
  const [templateType, setTemplateType] = useState('EMAIL'); // Default to EMAIL
  const [message, setMessage] = useState('');
  const [variable, setVariable] = useState('');
  const [selectedOption, setSelectedOption] = useState('message'); // Default to "message"
  const [addedItems, setAddedItems] = useState([]); // To store added messages/variables
  ...
  const handleAddMessage = () => {
    if (message) {
      setAddedItems([...addedItems, { type: 'message', content: message }]);
      setMessage(''); // Clear message input after adding
    }
  };

  const handleAddVariable = () => {
    if (variable) {
      setAddedItems([...addedItems, { type: 'variable', content: variable }]);
      setVariable(''); // Clear variable input after adding
      setSelectedOption('message'); // Automatically switch back to message input
    }
  };

  const handlePublish = () => {
    // Logic for handling publish action
    console.log("Publishing template:", { templateName, templateType, addedItems });
  };
...
{isModalOpen && (
  ...
  {/* Radio Buttons for Email and SMS */}
  ...
  {/* Conditional Message Input with Add Button */}
  ...
  {/* Conditional Variable Input with Add Button */}
  {selectedOption === 'variable' && (
    <div className="mb-4 flex items-center">
      <label className="block mb-1 text-sm font-medium text-gray-700">Variable</label>
      <input
        type="text"
    ...
    <ul className="list-disc ml-5">
      {addedItems.map((item, index) => (
        <li key={index} className="mb-2">
          {item.type === 'message' ? `Message: ${item.content}` : `Variable: ${item.content}`}
        </li>
      ))}
    </ul>
  </div>
  ...
  <div className="flex justify-between">
    ...
    <button
      ...
      disabled={!templateName || addedItems.length === 0}
    >
      Publish
    </button>
  </div>
