...
const PasswordResetForm = ({ email, onClose, onResetPassword }) => {
    ...
    return (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
            <div className="bg-white p-8 rounded-md shadow-lg">
                ...
                <p className="mb-4">Please enter a new password for your account.</p>
...
const Login = () => {
    ...
    const handleResetPassword = async (email, newPassword) => {
        try {
            const response = await axios.post('http://43.204.140.118:3001/reset-password', { email, newPassword });
            if (response.status) {
